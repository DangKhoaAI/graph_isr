...
def plot_curves(A=None, B=None, C=None, D=None, output_dir='output'):
    if not A:
        A = np.load(os.path.join(output_dir, 'epoch_training_losses.npy'))
        B = np.load(os.path.join(output_dir, 'epoch_training_scores.npy'))
        C = np.load(os.path.join(output_dir, 'epoch_test_loss.npy'))
        D = np.load(os.path.join(output_dir, 'epoch_test_score.npy'))
    ...
    plt.ylabel('accuracy')
    plt.legend(['train', 'test'], loc="upper left")
    title = os.path.join(output_dir, "curves.png")
